...
function scramblePixelValue(val, key) {
    const shuffledBitPositions = scramblePixelPosition(8);
    const bits = []
...
const encrypt = async flags => {
    ...
    try {
        ...
        await new Promise(resolve => {
            const scrambledPos = scramblePixelPosition(length);

            for (let i = 0; i < length; i++) {
                key.push(rgba[i])
                rgba[i] = scramblePixelValue(rgba[i], key);
            }

            for (let i = 0; i < length; i++) {
                new_rgba[i] = rgba[scrambledPos[i]];
            }
